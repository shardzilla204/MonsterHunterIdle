...
public partial class ItemBoxInterface : Container
{
	...
	// Add the container & then add the material logs
	private void AddMaterialLogs(List<Material> materials)
	{
		int childIndex = 0;
		Container materialLogContainer = GetMaterialLogContainer();
		FlowContainer flowContainer = materialLogContainer.GetChild<FlowContainer>(childIndex);
		foreach (Material material in materials)
		{
			MaterialLog materialLog = MonsterHunterIdle.PackedScenes.GetMaterialLog(material);
			flowContainer.AddChild(materialLog);
		}
		_parentContainer.AddChild(materialLogContainer);
	}
	...
	private ScrollContainer GetMaterialLogContainer()
	{
		ScrollContainer materialLogContainer = new ScrollContainer()
		{
			FollowFocus = true,
		...
		flowContainer.AddThemeConstantOverride("h_separation", separation);
		flowContainer.AddThemeConstantOverride("v_separation", separation);

		materialLogContainer.AddChild(flowContainer);
		return materialLogContainer;
	}
